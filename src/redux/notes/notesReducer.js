...
export default function notesReducer(state = INITIAL_STATE, action) {
  switch (action.type) {
    ...
    case types.DELETE_NOTE: {
      const newArray = [...state.notes].filter(
    ...
    case types.UPDATE_NOTE: {
      const newNoteArray = [...state.notes];
      const newObj = action.payload;
      const index = newNoteArray.findIndex((obj) => obj.id === newObj.id);

      newNoteArray.splice(index, 1, newObj);
      return {
        notes : newNoteArray
      }
    }
